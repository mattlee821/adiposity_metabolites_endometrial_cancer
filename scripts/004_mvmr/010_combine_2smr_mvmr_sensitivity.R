...
adiposity_sensitivity <- read.table("adiposity_metabolites_endometrial_cancer/analysis/001_adiposity_endometrial/female/exposure_data.txt", header = T, sep = "\t")
...
metabolite_sensitivity <- read.table("adiposity_metabolites_endometrial_cancer/analysis/003_metabolite_endometrial/female/exposure_data.txt", header = T, sep = "\t")
...
# mvmr sensitivity ====
data <- read.table("adiposity_metabolites_endometrial_cancer/analysis/004_mvmr/female/results/combined/combined_sensitivity.txt", header = T, sep = "\t")
...
write.table(sensitivity, "adiposity_metabolites_endometrial_cancer/analysis/004_mvmr/female/results/combined/2smr_mvmr_formatted_fstats.txt", 
...
# MR adiposity sensitivity ====
adiposity_sensitivity <- read.table("adiposity_metabolites_endometrial_cancer/analysis/001_adiposity_endometrial/combined/exposure_data.txt", header = T, sep = "\t")
adiposity_sensitivity <- unique(adiposity_sensitivity[,c("exposure", "mean_fstat")])
adiposity_sensitivity$method <- "Two-sample MR"

# MR metabolite sensitivity ====
metabolite_sensitivity <- read.table("adiposity_metabolites_endometrial_cancer/analysis/003_metabolite_endometrial/combined/exposure_data.txt", header = T, sep = "\t")
metabolite_sensitivity$exposure <- gsub("_int_imputed.txt_snps.txt", "", metabolite_sensitivity$exposure)
metabolite_sensitivity$exposure = gsub("_", "", metabolite_sensitivity$exposure)
metabolite_sensitivity$exposure = gsub("%", "pct", metabolite_sensitivity$exposure)
metabolite_sensitivity$exposure = gsub("-", "", metabolite_sensitivity$exposure)
...
# mvmr sensitivity ====
data <- read.table("adiposity_metabolites_endometrial_cancer/analysis/004_mvmr/combined/results/combined/combined_sensitivity.txt", header = T, sep = "\t")
mvmr_fstat <- data[,c("exposure", "outcome", "adjusted", "fstat.exposure1", "method")]
colnames(mvmr_fstat)[4] <- "mean_fstat"

# combine sensitivity
sensitivity <- bind_rows(adiposity_sensitivity, metabolite_sensitivity, mvmr_fstat)
write.table(sensitivity, "adiposity_metabolites_endometrial_cancer/analysis/004_mvmr/combined/results/combined/2smr_mvmr_formatted_fstats.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
